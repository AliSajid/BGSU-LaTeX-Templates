...
\DocumentMetadata{tagging=on, tagging-setup={activate/all}, lang=en-US, pdfversion=2.0, pdfstandard = ua-2}
\else
...
% Chapter quotes and epigraphs
\usepackage{epigraph}
\setlength{\epigraphwidth}{\textwidth}
\setlength{\epigraphrule}{0pt}

% Include PDF Files / Pages
\usepackage{pdfpages}
...
\usepackage{fontspec}
\setmainfont{Times New Roman}[Scale=1.00375] % Times-like, robust
...
% Poem Body: Forced Italic
\DefineVerbatimEnvironment{FoundPoem}{Verbatim}{
  formatcom={\rmfamily\itshape\onehalfspacing},
  fontsize=\small,
  obeytabs=true
}

% Environment for the poem title
\DefineVerbatimEnvironment{PoemTitle}{Verbatim}{
  formatcom={\rmfamily\itshape\onehalfspacing},
  fontsize=\small,
  obeytabs=true 
}
...
% Package to set up the Urdu typing
\usepackage{polyglossia}
\setmainlanguage{english}
\setotherlanguage{urdu}
%\newfontfamily\urdufont[Script=Arabic,Language=Urdu]{PakType Tehreer} % or Scheherazade 

\newcommand{\typeurdu}[1]{
(\texturdu{#1})
}
\newcommand{\justifyurdu}{\parfillskip=0pt \emergencystretch=.5\textwidth \par}

\raggedright   % BGSU requires no right justification

% Change the text inside each of the curly braces below ------------------------
\title{COMMUNICATION OF IDENTITY ACROSS FIRST, ONE-AND-A-HALF, AND SECOND-GENERATION PAKISTANI IMMIGRANT PARENTS AND CHILDREN THROUGH RETROSPECTIVE STORYTELLING}
\author{Saadia Farooq}
\degree{Doctor of Philosophy}
\date{2026}
\advisor{Sandra Faulkner}    % also known as committee chair
\gfr{Carrie M. Hamady}     % graduate faculty representative
\committee{Bailey Dick \\ \\ Lisa Hanasono}  % other committee members
\subject{Communication} % edit this for your area! Use broad terms
\keywords{Identity Negotiation; Retrospective Storytelling; Pakistani-American; Immigrant Family; Communication Theory of Identity (CTI); Art-based Research (ABR)} % be more specific here, separate keywords with semicolons

\setlength{\cftchapindent}{0in}      % Chapter numbers start at the margin
\setlength{\cftsecindent}{0.5in}     % Section numbers (1.1) indented 0.5in
\setlength{\cftsubsecindent}{1.0in}  % Subsection numbers (1.1.1) indented 1.0in
\begin{document}
